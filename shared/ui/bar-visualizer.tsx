...
export function useAudioVolume(
  ...
  useEffect(() => {
    ...
    try {
      const analyserSetup = createAudioAnalyser(mediaStream, memoizedOptions)
      cleanup = analyserSetup.cleanup
...
export function useMultibandVolume(
  ...
  useEffect(() => {
    ...
    let analyser: AnalyserNode
    try {
      const analyserSetup = createAudioAnalyser(mediaStream, opts.analyserOptions)
      cleanup = analyserSetup.cleanup
      analyser = analyserSetup.analyser
    } catch {
      const emptyBands = new Array(opts.bands).fill(0)
...
const BarVisualizerComponent = forwardRef<HTMLDivElement, BarVisualizerProps>(
  (
    ...
    // Audio processing — skip real analyser in demo mode
    const realVolumeBands = useMultibandVolume(demo ? null : mediaStream, {
      bands: barCount,
      loPass: 100,
      hiPass: 200,
    })
